...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen flex items-start md:items-center justify-center bg-background px-4 pt-8 sm:pt-14 md:pt-0 overflow-hidden relative">
      {/* Ambient orbs */}
      ...
      >
        ...
        <div className="p-5 sm:p-8 md:p-10 rounded-[2rem] bg-card/60 backdrop-blur-2xl border border-border/30 shadow-[0_8px_60px_-12px_rgba(0,0,0,0.08),0_0_0_1px_rgba(255,255,255,0.1)_inset]">
          ...
          {mode !== "forgot" && !showEmailForm && (
            <div className="space-y-3 sm:space-y-4">
              <motion.div whileHover={{ scale: 1.015 }} whileTap={{ scale: 0.985 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}>
                <Button
                  type="button"
                  className="w-full h-12 sm:h-14 rounded-2xl bg-[#1a73e8] hover:bg-[#1b66c9] text-white text-sm sm:text-base font-semibold shadow-[0_10px_30px_-8px_rgba(26,115,232,0.55)] hover:shadow-[0_16px_44px_-8px_rgba(26,115,232,0.75)] transition-all duration-300"
                  disabled={loading}
                  onClick={async () => {
                    setLoading(true);
                ...
                >
                  <span className="mr-2.5 flex items-center justify-center w-7 h-7 rounded-lg bg-white shadow-sm">
          ...
          <div className="mt-4 sm:mt-6 text-center text-sm">
            {mode === "login" && (
              <>
                {/* Forgot-password only surfaces once the email form is open. */}
                {showEmailForm && (
                  <button onClick={() => setMode("forgot")} className="text-accent hover:underline">{t.auth.forgotPassword}</button>
                )}
                <p className={`text-muted-foreground ${showEmailForm ? "mt-2" : ""}`}>
                  {t.auth.noAccount}{" "}
