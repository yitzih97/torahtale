...
Replace the current step 6 UI with a single, calm, two-level browser. Remove the "Weekly Parashah / Parashah+Holidays / Choose Manually" mode-picker entirely — it duplicates intent and confuses users.
...
```text
            ┌────────────────────────────────────────────┐
            │  Choose your story                          │
            │  [ This week's Parashah → Parashas Noach ]  │  ← primary suggested card
            └────────────────────────────────────────────┘

   [ Torah ] [ Nevi'im ] [ Ketuvim ] [ Megillos ] [ Yamim Tovim ]   ← single pill row
   ┌──────────────────────────────────────────────────────────────┐
   │ 🔍 Search parashah or story…                                 │
   └──────────────────────────────────────────────────────────────┘

   Sefer Bereishis / ספר בראשית            ▾
     Parashas Bereishis   Parashas Noach   Parashas Lech Lecha …
...
- **Remove** `portionMode` state and all three mode cards (`weeklyParashah`, `parashahAndHolidays`, `chooseManually`).
- **Always show** the manual browser; planType no longer gates UI.
- **Top "This week's Parashah" hero card**: always rendered above the pills, showing the upcoming parshah from `getUpcomingParsha()` with its proper "Parashas X / פרשת X" label and a single tap selects it + auto-advances. Replaces the current weekly card duplication.
...
- **Parsha tiles**: keep the existing card style but show the new "Parashas X" label and the Hebrew sub. Selection still triggers `autoAdvance()`.
...
- Add/update strings: `thisWeekParshah` ("This week's Parashah" / "פרשת השבוע" / "די וואכעדיגע פרשה"), `seferTorah` etc only as needed.
- Remove now-unused keys: `weeklyParashah`, `weeklyParashahDesc`, `parashahAndHolidays`, `parashahAndHolidaysDesc`, `chooseManually`, `chooseManuallyDesc`, `backToOptions`, `storyStartTitleSingle`, `storyStartTitleSubscription`, `all`, `sefer` (replaced by inline bilingual labels).
